...
- Read everything: `GET /projects`, `GET /projects/:id`, `GET /business-context?project_id=...`.
- Change settings: `PUT /projects/:id` with only the fields to change. Editable: `ai_instructions`, `business_description`, `publish_time`, `timezone`, `backlinks_network`. Note: `articles_per_day` is plan-controlled and cannot be changed via the API.
